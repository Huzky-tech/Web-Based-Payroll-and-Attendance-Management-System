...
if ($method === 'GET') {
    ...
    $where = $scope !== ''
        ? "WHERE ({$scope}) AND {$activeEmployeeCondition}"
    ...
    $sql = "SELECT 
                ...
                wp.StreetAddress AS street_address,
                wp.City AS city,
                wp.StateProvince AS state_province,
                wp.PostalCode AS postal_code,
                wp.Country AS country,
                wa.SiteID,
                ps.Site_Name,
    ...
    $result = employee_query($conn, $sql, $types, $params);
    
    $employees = [];
    while ($result && $row = $result->fetch_assoc()) {
        if (empty($row['qr_code_path'])) {
            try {
                $row['qr_code_path'] = ensureEmployeeQrCode(
                    $conn,
                    (int) $row['WorkerID'],
                    $row['qr_code_path'] ?? null
                );
