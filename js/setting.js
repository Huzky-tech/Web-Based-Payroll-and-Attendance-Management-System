...
const payrollNumericRules = {
    philhealth_rate: { label: 'PhilHealth Rate', min: 0, max: 100 },
    pagibig_rate: { label: 'Pag-IBIG Rate', min: 0, max: 100 },
    overtime_rate: { label: 'Overtime Rate', min: 0.01 },
    late_worker_deduction: { label: 'Late Worker Deduction', min: 0 }
};

function setPayrollFieldError(input, message = '') {
    if (!input) return;
    let error = input.parentElement?.querySelector('.settings-field-error');
    ...
    input.classList.toggle('settings-input-invalid', Boolean(message));
    if (error) error.textContent = message;
}

function getPayrollFieldError(input, rule) {
    const value = String(input?.value || '').trim();
    if (value === '') return `${rule.label} is required.`;
    // A trailing decimal point is valid while the user is still typing, e.g. "3."
    // before completing "3.00".
    if (!/^\d+(?:\.\d*)?$/.test(value)) return `${rule.label} must contain numbers and one decimal point only.`;
    const number = Number(value);
    if (number < rule.min) return `${rule.label} must be ${rule.min === 0 ? '0 or greater' : `at least ${rule.min}`}.`;
    if (typeof rule.max === 'number' && number > rule.max) return `${rule.label} cannot be greater than ${rule.max}.`;
    return '';
}

function setupPayrollLiveValidation() {
    Object.entries(payrollNumericRules).forEach(([id, rule]) => {
        const input = document.getElementById(id);
        if (!input) return;
        input.setAttribute('inputmode', 'decimal');
        input.addEventListener('input', () => {
            const original = input.value;
            // Permit digits plus a single decimal point.  Do not flag the
            // decimal point itself as a special character.
            const digitsAndDots = original.replace(/[^\d.]/g, '');
            ...
            setPayrollFieldError(input, getPayrollFieldError(input, rule));
        });
        input.addEventListener('blur', () => setPayrollFieldError(input, getPayrollFieldError(input, rule)));
    });
}

function setupGeneralSettingsLiveValidation() {
    const textRules = {
    ...
    Object.entries(textRules).forEach(([id, rule]) => {
        ...
        input.addEventListener('input', () => {
            const original = input.value;
            input.value = original.replace(rule.pattern, '');
            setPayrollFieldError(input, original !== input.value ? `${rule.label} cannot contain special characters.` : '');
        });
    });

    ['password_expiry_days', 'min_password_length', 'max_login_attempts', 'session_timeout_minutes', 'data_retention_value'].forEach((id) => {
        const input = document.getElementById(id);
        if (!input) return;
        input.addEventListener('input', () => {
            const original = input.value;
            input.value = original.replace(/\D/g, '');
            const message = original !== input.value
                ? 'Only whole numbers are allowed. Special characters and letters are not allowed.'
                : (!input.value ? 'This field is required.' : (Number(input.value) < 1 ? 'Value must be at least 1.' : ''));
            setPayrollFieldError(input, message);
        });
    });
}
...
function validatePayrollSettings() {
    ...
    Object.entries(payrollNumericRules).forEach(([id, rule]) => {
        ...
        const message = getPayrollFieldError(input, rule);
        setPayrollFieldError(input, message);
...
async function loadSecuritySettings() {
    ...
    userPasswordPolicy = { ...userPasswordPolicy, ...data.data };
    if (document.getElementById('newUserPassword')) validatePasswords();
...
async function loadAllSettings() {
    ...
    if (failed.length) {
        failed.forEach(({ label, result }) => console.error(`Failed to load ${label} settings:`, result.reason));
        // The remaining tabs still work, and the browser console identifies the
        // specific endpoint if a database table or API response needs attention.
        console.warn(`Unable to load: ${failed.map(({ label }) => label).join(', ')} settings.`);
    }
}
...
function validateUserIdentityFields(mode, showRequired = false) {
    ...
    const namePattern = /^(?=.{1,50}$)[\p{L}]+(?: [\p{L}]+)*$/u;
    ...
    const firstValid = validate(first, `${prefix}FirstNameError`, namePattern, 'first name');
    const lastValid = validate(last, `${prefix}LastNameError`, namePattern, 'last name');
    const emailValid = validate(email, `${prefix}EmailError`, emailPattern, 'email address');
    const currentId = mode === 'edit' ? Number(document.getElementById('editUserId')?.value) : 0;
    const otherUsers = userAllUsersData.filter(user => Number(user.id) !== currentId);
    const duplicateName = firstValid && lastValid && otherUsers.some(user =>
        String(user.full_name || '').trim().replace(/\s+/gu, ' ').toLowerCase() === `${first.value.trim()} ${last.value.trim()}`.toLowerCase());
    const duplicateEmail = emailValid && otherUsers.some(user =>
        String(user.email || '').trim().toLowerCase() === email.value.trim().toLowerCase());
    const markDuplicate = (field, messageId, text) => {
    ...
    if (duplicateName) {
        markDuplicate(first, `${prefix}FirstNameError`, 'This first and last name combination is already registered.');
        markDuplicate(last, `${prefix}LastNameError`, 'This first and last name combination is already registered.');
    }
    if (duplicateEmail) markDuplicate(email, `${prefix}EmailError`, 'This email is already registered.');
    if (!emailValid || duplicateEmail) {
        window.resetUserEmailAvailability?.(mode);
        email?.setCustomValidity('Enter an available email address.');
    }
    const emailAvailable = emailValid && !duplicateEmail && (window.checkUserEmailAvailability?.(mode) ?? false);
    const isValid = firstValid && lastValid && emailValid && !duplicateName && !duplicateEmail && emailAvailable;
    if (button) button.disabled = button.dataset.saving === 'true' || !isValid;
    return Boolean(isValid);
}

function newUserPasswordError(password) {
    const minimum = Math.max(1, Number(userPasswordPolicy.min_password_length) || 8);
    if (new TextEncoder().encode(password).length < minimum) return `Password must be at least ${minimum} characters long.`;
    const required = id => Number(userPasswordPolicy[id]) === 1;
    if (required('require_special_char') && !/[^A-Za-z0-9]/.test(password)) return 'Password must include a special character.';
    if (required('require_number') && !/[0-9]/.test(password)) return 'Password must include a number.';
    if (required('require_uppercase') && !/[A-Z]/.test(password)) return 'Password must include an uppercase letter.';
    return '';
}
...
function validatePasswords() {
    ...
    const policyError = newUserPasswordError(password);
    if (password && policyError) {
        passwordField.classList.add('password-error');
        passwordError.textContent = policyError;
...
async function fetchUsersList() {
    try {
        ...
        if (document.getElementById('newUserFirstName')) validateUserIdentityFields('new');
        userCurrentPage = 1;
        applyUserFilters();
...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    // Employee is a standalone user-management page. Do not initialize the
    // Company, Payroll, Notification, Security, or System Settings forms here.
    if (window.isUserManagementPage === true || document.getElementById('users')?.dataset.standalone === 'true') {
        await loadUsers();
        return;
    }
    ...
    document.getElementById('debug_mode')?.addEventListener('change', () => updateSampleDataDevToolsVisibility());

    handleOvertimeToggle();
    setupPayrollLiveValidation();
    setupGeneralSettingsLiveValidation();
